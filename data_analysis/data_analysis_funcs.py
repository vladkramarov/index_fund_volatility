...
def plot_autocorrelations(data: pd.DataFrame = train_data, tickers: List[str] = core.TICKERS, target_feature = training.config.TARGET, lags: int = 50):
    fig, ax = plt.subplots(len(tickers), 2, figsize=(12, 12))
    for ax_row, ticker in zip(ax, tickers):
        ax1, ax2 = ax_row
        sm.graphics.tsa.plot_acf(data.query(f'ticker == "{ticker}"')[target_feature], lags=lags, ax=ax1, zero = False)
        sm.graphics.tsa.plot_pacf(data.query(f'ticker == "{ticker}"')[target_feature], lags=lags, ax=ax2, zero = False)
