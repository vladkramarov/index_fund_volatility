...
def compare_train_valid_test_splits(ticker: str):
    ...
    sns.kdeplot(
        data=train.query(f'ticker == "{ticker}"')[training.config.TARGET],
        ax=ax,
        label="train",
        fill=True,
        alpha=0.2,
    ...
    sns.kdeplot(
        data=valid.query(f'ticker == "{ticker}"')[training.config.TARGET],
        ax=ax,
        label="valid",
        fill=True,
        alpha=0.2,
    ...
    sns.kdeplot(
        data=test.query(f'ticker == "{ticker}"')[training.config.TARGET],
        ax=ax,
        label="test",
        fill=True,
        alpha=0.2,
...
def get_distribution_statistics(
    data: pd.DataFrame = train_data, ticker: str = "AAPL"
...
def plot_autocorrelations(
    ...
    target_feature=training.config.TARGET,
    lags: int = 50,
):
    ...
    for ax_row, ticker in zip(ax, tickers):
        ...
        sm.graphics.tsa.plot_acf(
            data.query(f'ticker == "{ticker}"')[target_feature],
            lags=lags,
            ax=ax1,
            zero=False,
        )
        ...
        ax2.text(
            0.05,
            0.95,
            f"ADF: {get_adf(data, ticker)[1]:.3e}",
...
def violin_plots(data: pd.DataFrame = train_data, hue_feature: str = "ticker"):
    ...
    if hue_feature == "ticker":
        sns.violinplot(
            data=data, y="ticker", x=training.config.TARGET, ax=ax, orient="h"
        )
    elif hue_feature == "month" or hue_feature == "day_of_week":
        data_by_date = (
            data.groupby("date")[[training.config.TARGET, hue_feature]]
            .mean()
            .reset_index()
        )
        sns.violinplot(
