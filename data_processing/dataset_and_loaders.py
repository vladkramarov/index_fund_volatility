...
def get_timeseries_datasets():
    ...
    training_dataset = TimeSeriesDataSet(
        ...
        target_normalizer=GroupNormalizer(
            groups=training.config.GROUP_IDS, transformation="softplus"
        ),
...
def get_encoder_data(last_predicted_dataset: pd.DataFrame):
    encoder_start_idx = (
        last_predicted_dataset.groupby("ticker")["idx"].max().min()
        - training.config.MAX_ENCODER_LENGTH
    )
...
def get_extra_max_prediction_length_rows(data: pd.DataFrame):
    ...
    return pd.concat(
        [
            last_row_per_team.assign(idx=lambda row: row.idx + i)
            for i in range(1, training.config.MAX_PREDICTION_LENGTH)
        ]
    )
...
def get_test_dataset_and_dataloaders(train_dataset: TimeSeriesDataSet):
    ...
    test_dataloader = test_dataset.to_dataloader(
        train=False,
        batch_size=training.config.VAL_BATCH_SIZE,
        num_workers=training.config.NUM_WORKERS,
        shuffle=False,
    )
