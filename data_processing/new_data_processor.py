...
def get_last_market_date(
    date: Union[dt.datetime, str] = None, market_schedule: pd.DataFrame = None
):
    ...
    past_market_dates = market_schedule.loc[
        market_schedule["market_date"] < todays_date
    ]
...
class NewDataManager:
    def __init__(
        self, tickers: List[str], prediction_start_date: Union[dt.datetime, str]
    ):
        self.tickers = tickers
        self.prediction_start_date = prediction_start_date
        self._market_schedule = loader.get_market_schedule()
        self._todays_date, self.last_market_date, self._past_market_dates = \
            get_last_market_date(
                date=self.prediction_start_date, market_schedule=self._market_schedule)

    def _get_encoder_start_date(self):
        prediction_start_date = pd.to_datetime(self.prediction_start_date).strftime(
            "%Y-%m-%d"
        )
        ...
        self.encoder_start_date = pd.to_datetime(
            np.busday_offset(
                prediction_start_date, -business_days_to_subtract, roll="backward"
            )
        )

    def _get_starting_idx_value(self):
        encoder_start_date_str = pd.to_datetime(self.encoder_start_date).strftime(
            "%Y-%m-%d"
        )
    ...
    def _get_first_prediction_idx(self):
        if self.processed_data is None:
            ...
            self.first_prediction_idx = self.processed_data.loc[
                self.processed_data["date"] == self.prediction_start_date
            ]["idx"].min()
    ...
    def _get_new_data(self):
        self.new_data = loader.get_data_from_api(
            start_date=self.encoder_start_date, tickers=self.tickers
        )
        self.new_data["Date"] = self.new_data["Date"].dt.tz_localize(None)
        self.new_data = self.new_data.loc[
            self.new_data["Date"] <= self.last_market_date
        ]
        self.new_data["mask"] = 0

    def _get_future_dates_for_predictions(self):
        last_market_index = self._market_schedule.loc[
            self._market_schedule["market_date"] == self.last_market_date
        ].index[0]
    ...
    def _get_future_rows(self):
        ...
        self.future_rows = pd.concat(
            [
                last_data_point_per_ticker.assign(
                    Date=lambda x: self.future_prediction_dates[i]
                )
                for i in range(len(self.future_prediction_dates))
            ]
        )
    ...
    def _process_data(self):
        self.processed_data = data_formatter.data_management_pipeline(
            self.new_data, self.starting_idx
        )
...
def new_data_pipeline(
    tickers: List[str], prediction_start_date: Union[dt.datetime, str]
):
...
def get_timeseries_dataset(processed_data: pd.DataFrame):
    ...
    processed_ts_dataset = TimeSeriesDataSet.from_parameters(
