...
def process_output(preds, prediction_start_date: str, processed_data: pd.DataFrame, prediction_data_type: str = 'list') -> pd.DataFrame:
    output_form = output_formatter.OutputFormatter(earliest_prediction_date=prediction_start_date, prediction=preds, prediction_data_type=prediction_data_type)
    return output_form.get_unaligned_results(processed_data)
    
def prediction_pipeline(tickers = ['XLK'], prediction_start_date = '2023-08-09') -> pd.DataFrame:
