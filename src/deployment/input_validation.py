...
class InputData(BaseModel):
    ...
    def validate_tickers(cls, tickers):
        if any(ticker not in core.TICKERS for ticker in tickers):
            raise ValueError(
                "Invalid ticker. Please select a ticker from the list of available tickers.\nAvailable tickers are: {}".format(
                    core.TICKERS
                )
            )
        return tickers
    ...
    def validate_dates(cls, prediction_start_date):
        try:
            prediction_start_date = dt.datetime.strptime(
        ...
        if (
            ...
            not in market_schedule["market_date"].values
        ):
            raise ValueError(
                "Prediction start date must be a market day. Select a different date"
            )
            ...
            raise ValueError(
                "Prediction start date must be within 5 market days of the last market date"
            )
