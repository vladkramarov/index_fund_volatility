...
def split_train_valid_test(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    train_end_idx = (
        max_idx - config.VALIDATION_DATA_LENGTH - config.TEST_DATA_LENGTH)
    valid_end_idx = max_idx - config.TEST_DATA_LENGTH
...
def generate_election_date_ranges(election_dates: List[str] = core.ELECTION_DATES) -> List[str]:
