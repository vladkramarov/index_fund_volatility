...
class DataProcessor:
    ...
    def get_historical_volatility(self):
        if "log_returns" in self.data.columns:
            self.data[f"historical_volatility_5_day"] = (
    ...
    def get_previous_day_range(self, high_price: str = "high", low_price: str = "low"):
        if high_price and low_price in self.data.columns:
            self.data["log_prev_day_range"] = np.log(
                self.data.groupby(self.identifier)[high_price].shift(1)
    ...
    def fill_blank_target_rows(self):
        ...
        """
        self.data[["volatility_target", "volatility"]] = self.data[
    ...
    def add_earnings_periods(self):
        self.data["earnings_period_new"] = self.data.apply(
            lambda row: 1.0
            if row["day_of_the_month"] in np.arange(10, 22, 1)
            and row["month"] in [1, 4, 7, 10]
            and row["day_of_the_week"] != 4
            else 0,
            axis=1,
        )
    ...
    def add_cpi_report(self):
        ...
        self.data["date"] = pd.to_datetime(self.data["date"]).dt.date.astype(
            "datetime64[ns]"
        )
        self.data = pd.merge(
            self.data,
        ...
        self.data["cpi_date"] = self.data["cpi_date"].apply(
            lambda x: 0 if pd.isnull(x) else 1
        )
        self.data.rename(columns={"cpi_date": "cpi_report"}, inplace=True)

    def positive_or_negative_returns(self):
        self.data["positive_or_negative"] = self.data["log_returns"].apply(
    ...
    def monthly_options_expiration(self):
        self.data["monthly_options_expiration"] = self.data.apply(
            lambda row: 1
            if row["day_of_the_week"] == 4 and row["weekday_count"] == 3
            else 0,
            axis=1,
        )
    ...
    def add_election_date_ranges(self):
        ...
        self.data["election_dates"] = self.data.apply(
            lambda row: 1 if row["date"].strftime("%Y-%m-%d") in election_dates else 0,
            axis=1,
        )

    def employment_report(self):
        self.data["employment_report"] == self.data.apply(
            lambda row: 1
            if row["day_of_the_week"] == 4 and row["weekday_count"] == 1
            else 0,
            axis=1,
        )
