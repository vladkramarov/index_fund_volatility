...
class NewDataManager:
    ...
    def __init__(self, tickers: List[str], prediction_start_date: Union[dt.datetime, str]):
        ...
        self._todays_date, self.last_market_date, self._past_market_dates = \
            get_last_market_date(date=self.prediction_start_date, market_schedule=self._market_schedule)

    def _get_encoder_start_date(self):
        prediction_start_date = pd.to_datetime(self.prediction_start_date).strftime(
            "%Y-%m-%d")
        business_days_to_subtract = (config.MAX_ENCODER_LENGTH + config.VOLATILITY_WINDOW + 5)
    ...
    def _get_starting_idx_value(self):
        ...
        business_days = np.busday_count(config.LAST_TEST_DATASET_DATE, encoder_start_date_str)
    ...
    def _get_future_dates_for_predictions(self):
        last_market_index = self._market_schedule.loc[
            self._market_schedule["market_date"] == self.last_market_date].index[0]
        self.future_prediction_dates = self._market_schedule.loc[last_market_index + 1: last_market_index + config.MAX_PREDICTION_LENGTH - 1]["market_date"].values

    def _get_future_rows(self):
        last_data_point_per_ticker = self.new_data.groupby("ticker").tail(1)
        self.future_rows = pd.concat(
            [last_data_point_per_ticker.assign(Date=lambda x: self.future_prediction_dates[i])
